...
export function AdminLoanApplications() {
  ...
  const filteredApplications = filterStatus === "All"
    ...
    : loans.filter(app => {
      return matchesFilterStatus(app.status, filterStatus) &&
        (app.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          app.category.toLowerCase().includes(searchTerm.toLowerCase()))
    });
                  ...
                  {filteredApplications.map((loanApp, idx) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 text-sm whitespace-nowrap">
                        <span className={`inline-flex items-center px-3 py-1 rounded text-xs font-bold ${getStatusBadgeClass(loanApp.status)}`}>
                          {getDisplayStatus(loanApp.status)}
                        </span>
                      </td>
                    </tr>
                  ))}
